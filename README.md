...
Short-Term Pacing (Leaky Bucket): Enforces a strict ~55ms interval between outgoing requests to prevent micro-bursts.

Long-Term throttling (Priority Token Bucket): Manages the global quota (e.g., 100 requests/2 minutes).
...
Resolution: getAccountByRiotId resolves Name#Tag to a global PUUID (Account-V1).

Validation: The system verifies the PUUID exists on the requested specific region (Summoner-V4). This prevents returning data for a user who exists globally but doesn't play on the queried server.

Cache "Always-Check" Strategy:
